...
const Tables = () => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (search === "" || search === null || search === undefined) return null;
    axios
  ...
  const goToPayment = (idStudent, idEducationOfStudent) => {
    localStorage.setItem("idStudent", idStudent);
    localStorage.setItem("idEducationOfStudent", idEducationOfStudent);

    navigate("/financial/payment");
